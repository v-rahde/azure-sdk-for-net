...
namespace Relay.Tests.ScenarioTests
{
    ...
    public partial class ScenarioTestsRelay 
    {
            ...
            if (!m_initialized)
            {
                lock (m_lock)
                {
                    if (!m_initialized)
                    {
                        ...
                        _RelayManagementClient = ServiceBusManagementHelper.GetServiceBusManagementClient(context, new RecordedDelegatingHandler { StatusCodeToReturn = HttpStatusCode.OK });
                    }
                }
            }
        }
        ...
        protected void TryCreateNamespace()
        {
            ...
            this.Location = ServiceBusManagementHelper.DefaultLocation;

            if (string.IsNullOrWhiteSpace(ResourceGroupName))
            {
                ResourceGroupName = TestUtilities.GenerateName(ServiceBusManagementHelper.ResourceGroupPrefix);
                this.ResourceManagementClient.TryRegisterResourceGroup(Location, ResourceGroupName);
            }

            NamespaceName = TestUtilities.GenerateName(ServiceBusManagementHelper.NamespacePrefix);
            this.RelayManagementClient.TryCreateNamespace(ResourceGroupName, NamespaceName, Location);
        }        
    }
}
